...
import { CalendarDays, Clock3, Globe, MapPin, Mic, MicOff, Monitor, Plus, Search, Send, Users, Video, VideoOff, X, ChevronRight, ChevronLeft, Circle, CircleDot, Pencil, Trash2, Pin, PinOff, UserX, Image, Check, MessageCircle } from 'lucide-react'
...
function SessionDetail({session,userId,isHealer,mine,onBack,onJoinRoom,onOpenChat,onRegister,onCancelSession,onEdit,onUploadCover,load}:{session:SessionRow;userId:string;isHealer:boolean;mine:{session_id:string;status:string}[];onBack:()=>void;onJoinRoom:(id:string)=>void;onOpenChat:(id:string)=>void;onRegister:(s:SessionRow)=>void;onCancelSession:(s:SessionRow)=>void;onEdit:(id:string)=>void;onUploadCover:(sessionId:string,file:File)=>void;load:()=>void}){
...
function LiveRoom({session,userId,isHost,onClose}:{session:SessionRow;userId:string;isHost:boolean;onClose:()=>void}){
  ...
  const [roomStatus,setRoomStatus]=useState<'waiting'|'live'|'ended'>(session.session_room_state?.status==='live'?'live':'waiting')
  ...
  useEffect(()=>{
    ...
    provider.onEvents({
      ...
      onRoomStateChange:(state)=>{setRoomStatus(state.status);if(state.status==='ended')onClose()},
