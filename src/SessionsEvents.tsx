...
function LiveRoom({session,userId,isHost,onClose}:{session:SessionRow;userId:string;isHost:boolean;onClose:()=>void}){
  ...
  const [roomStatus,setRoomStatus]=useState<'waiting'|'live'|'ended'>(session.session_room_state?.status==='live'?'live':session.session_room_state?.status==='ended'?'ended':'waiting')
