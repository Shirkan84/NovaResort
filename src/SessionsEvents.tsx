...
export function SessionsPage({userId,isHealer,onClose,initialSessionId,initialSessionView}:{userId:string;isHealer:boolean;onClose:()=>void;initialSessionId?:string|null;initialSessionView?:string|null}) {
  ...
  const load=useCallback(async()=>{
    ...
    const [{data,error},{data:registrations,error:regError}]=await Promise.all([
      supabase.from('sessions').select('*,profiles!sessions_host_id_fkey(full_name,display_name,avatar_url,profile_type,specialties),session_registrations(status,user_id),session_room_state(status,started_at,ended_at)').order('starts_at',{ascending:true}).limit(80),
